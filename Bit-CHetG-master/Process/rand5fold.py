...
def load5foldData(obj):
    ...
    if obj == "Bitcoin":
        labelPath = os.path.join(cwd,"data/Bitcoin/elliptic_5_01T_id_label.txt")
...
def load1foldData(obj):
    if obj == "Bitcoin":
        labelPath = os.path.join(cwd,"data/Bitcoin/bitcoin_id_label.txt")
        print("loading bitcoin label:")
        F, T = [], []
        l1 = l2 = 0
        labelDic = {}
        for line in open(labelPath):
            ...
            eid,label = line.split(' ')[0], line.split(' ')[1]
            # labelDic[eid] = int(label)
            # labelDic[eid] = round(float(label),3)
            #F是合法，0
        ...
        leng1 = int(l1 * 0.02)
        leng2 = int(l2 * 0.2)
        F=sample(F, int(l1 * 0.1))
        Ftest=sample(F, leng1)
        Ftrain=list(set(F)-set(Ftest))
        # Ftrain=[i for i in F if i not in Ftest]
        # T=sample(T, int(l2 * 0.01))
        Ttest=sample(T, leng2)
        # Ttrain=[i for i in T if i not in Ttest]
        Ttrain=list(set(T)-set(Ttest))
        fold0_x_test.extend(Ftest)
        fold0_x_test.extend(Ttest)
        fold0_x_train.extend(Ftrain)
        fold0_x_train.extend(Ttrain)

    fold0_test = list(fold0_x_test)
    shuffle(fold0_test)
    fold0_train = list(fold0_x_train)
    shuffle(fold0_train)
    print("test no:", len(fold0_test))
    print("train no:", len(fold0_train))
